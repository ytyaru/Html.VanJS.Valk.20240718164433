...
class HookAry {
    ...
    constructor(ary, options={}, insOpts={}, hookableMethodNames=[], notHookableMethodNames=[]) {
        ...
        //this._hookableMethodNames = (Type.isAry(this._hookableMethodNames) && 0<this._hookableMethodNames.length) ? this._hookableMethodNames : destroyMethods.Array);
        //this._notHookableMethodNames = (Type.isAry(this._notHookableMethodNames) && 0<this._notHookableMethodNames.length) ? this._notHookableMethodNames : notDestroyMethods.Array);
        this._options = {...{
        ...
        */
        ...
        this._defineMethods()
        //return InsProxy.of(this, {...target, ...insOpts})
        return InsProxy.of(this, {
            ...
            /*
            onGetDefined:(target, key, receiver)=>{
                if (Type.isInt(key) && key in this._v) { if (this._options.getUndefined) { return this._v[key] } }
                // 破壊的メソッドの取得（呼出）禁止
//                const con = this._getContainer(target)
//                if (con && !this._options.getDestroyMethod 
//                    && destroyMethods[con.name].some(n=>n===key)) {throw new FixError()} 
            ...
            onGetUndefined:(target, key, receiver)=>{
                if (this.#isNumeric(key)) {
                    ...
                    if (0<=idx && idx<this._v.length) { return this._v[idx] }
                    else {throw new TypeError(`定義済プロパティへの参照禁止: ${key}`)}
                }
                //console.log(key, Type.isInt(key), 0<=key, key<this._v.length )
                //if (Type.isInt(key) && this._options.getUndefined) { return this._v[key] }
                //if (Type.isInt(key) && 0<=key && key<this._v.length) { return this._v[key] }
                else if (this._options.getUndefined) { return target[key] }
//                if (key in this._v) { if (this._options.getUndefined) { return this._v[key] } }
//                if (this._options.getUndefined) { return target[key] }
//                if (this._options.getUndefined) {
//                    if (key in this._v)  { return this._v[key] }
//                    return target[key]
//                }
                else { throw new TypeError(`未定義プロパティへの参照禁止: ${key}`) }
            },
            */
            ...
            onSetUndefined:(target, key, value, receiver)=>{
                if (this.#isNumeric(key)) {
                    const idx = parseInt(key)
//                    if (0<=idx && idx<this._v.length) { return this._v[idx] }
                    if (this._options.setDefined) { target[key] = value }
                    else {throw new TypeError(`定義済プロパティへの代入禁止: ${key}`)} // HookAryでは未定義だがthis._vでは定義済
                }
                if (this._options.setUndefined) { target[key] = value }
                else { throw new TypeError(`未定義プロパティへの代入禁止: ${key}`) }
                return true
    ...
    get(target, key, receiver) {
        ...
        // 破壊的メソッドの取得（呼出）禁止
//        const con = this._getContainer(target)
        //if (con && !this._options.getDestroyMethod && destroyMethods[con.name].some(n=>n===key)) {throw new FixError()} 
        //if (con && !this._insOpts.getDestroyMethod && destroyMethods[con.name].some(n=>n===key)) {throw new FixError()} 
        //if (!this._insOpts.getDestroyMethod && destroyMethods[con.name].some(n=>n===key)) {throw new FixError()} 
...
//        if (key in target) { return this.get(target, key) }
//        else {throw new TypeError(`未定義プロパティへの参照禁止: ${key}`)}
        if (key in target) {
            if (Type.hasGetter(target, key)) { return Reflect.get(target, key) } // getter
            else if (Type.isFn(target[key])) { return target[key].bind(target) } // method, constructor?
    ...
    set(target, key, value, receiver) {
       if (Type.isStr(key) && this.#isNumeric(key)) {
        ...
        //this._options[o===n ? 'onUnchanged' : 'onChanged'](target, key, value, receiver, n, o)
        //this._options[o.eq(n) ? 'onUnchanged' : 'onChanged'](target, key, value, receiver, n, o)
    ...
    _defineMethods() {
        ...
        //const names = (Type.isAry(this._hookableMethodNames) && 0<this._hookableMethodNames.length) ? this._hookableMethodNames : destroyMethods.Array);
...
//                    const o = Type.toStr(this) // ディープコピー（全文字列化）
                    ...
                    //console.log(name, n, o)
                    //this._options[o===n ? 'onUnchanged' : 'onChanged'](this, name, args, o)
                    //this._options[o.eq(n) ? 'onUnchanged' : 'onChanged'](this, name, args, receiver, n, o)
