    ...
    static _getProxy(obj) {
        return new Proxy(obj, {
            ...
            get(target, key, receiver) {
                if ('typeName'===key) { return `Proxy(valk.FixedValue)` }
    ...
    */
    ...
    static _getProxy(obj, fixErrFnNms=[]) { // fixErrFnNms:破壊的メソッド名リスト
        ...
        return new Proxy(obj, {
            ...
            get(target, key, receiver) {
                //if ('typeName'===key) { return `Proxy(valk.FixedValue)` }
                if ('typeName'===key) { return `Proxy(valk.Fixed${nm})` }
...
//                else { throw new ReferenceError(`Property does not exist: ${key}`) }
    ...
    static getTypeNameId(v) {
        if (Array.isArray(v) || v instanceof Set || v instanceof Map) { return v.constructor.name }
//        if ('Object'===v.constructor.name) { return 'Map' }
        return 'Value'
    }
}
...
class FixedArray extends ChangedArray {
    ...
    _getProxy(...args) {
        return new Proxy(...args, {
            set(target, key, value, receiver) { throw new FixError() },
            get(target, key, receiver) {
                if ('typeName'===key) { return `Proxy(valk.FixedArray)` }
...
class FixedSet extends ChangedSet {
    ...
    _getProxy(...args) {
        ...
        //return new Proxy(...args, {
            set(target, key, value, receiver) { throw new FixError() },
            get(target, key, receiver) {
                if ('typeName'===key) { return `Proxy(valk.FixedSet)` }
                if ('add,clear,delete'.split(',').some(n=>n===key)) { throw new FixError() }
                if (key in target) {
                    if (Type.hasGetter(target, key)) { return Reflect.get(target, key) }
                    else if ('function'===typeof target[key]) { return target[key].bind(target) }
...
//        if ('Object'===v.constructor.name) { return new FixedMap(Object.entries(v)) }
        ...
        //return new Proxy(...args, {
            set(target, key, value, receiver) { throw new FixError() },
            get(target, key, receiver) {
                if ('typeName'===key) { return `Proxy(valk.FixedMap)` }
