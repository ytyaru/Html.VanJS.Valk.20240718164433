...
//        p('A JavaScript library that controls value assignment using callback functions.'),
    ...
    a.t(()=>{
        ...
        return `Proxy(valk.FixedValue)`===zero.typeName
    })
    ...
    a.t('Proxy(valk.FixedValue)'===valk.fix(0).typeName)

    // FixedArray
    a.t(Array.isArray(valk.fix([1,2,3])))
    a.t(()=>{
        const fixAry = valk.fix([1,2,3])
        return 'Proxy(valk.FixedArray)'===fixAry.typeName
    })
    a.t(()=>{
    ...
    a.t('Proxy(valk.FixedMap)'===valk.fix(new Map([['k', 1], ['l', 2]])).typeName)
    a.t('Proxy(valk.FixedMap)'===valk.fix({k:1, l:2}).typeName)
